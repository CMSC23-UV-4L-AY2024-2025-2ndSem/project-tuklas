...
class _UserProfilePageState extends State<UserProfilePage> {
  ...
  Future<void> _loadUserProfile() async {
    if (!mounted) return; // in case this was called too early
...
Future<void> _saveProfile() async {
  ...
  try {
    ...
    // Update profile image only if there's a new image
    if (base64Image != null) {
      ...
      if (success) {
        ...
        print('Failed to update profile image. Please try again.');
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to update profile image')),
        );
        return; // Stop further execution if image update fails
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Profile', style: TextStyle(color: Color(0xFF027572), fontWeight: FontWeight.bold)),
        centerTitle: true,
        actions: [
          IconButton(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const Spacer(),
                        ElevatedButton(
                          style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF027572)),
                    ...
                    if (_isEditing) ...[
                      ...
                      Text('@${widget.username}', style: GoogleFonts.poppins(color: Colors.black54)),
                      const SizedBox(height: 4),
    ...
    return Wrap(
      ...
      children: options.map((option) {
        ...
        return ChoiceChip(
          ...
          labelStyle: GoogleFonts.poppins(color: selectedFlag ? Colors.white : Colors.black),
        );
      }).toList(),
    );
  }
